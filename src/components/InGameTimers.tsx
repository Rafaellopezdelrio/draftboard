...
function etaFor(
  ...
  if (row.respawnSec === 0) {
    if (killedAt !== null) return { etaSec: 0, ready: false, label: t("liveTimers.dead") };
    const eta = row.firstSpawnSec - gameTime;
    return { etaSec: eta, ready: eta <= 0, label: eta <= 0 ? t("liveTimers.now") : formatMMSS(eta) };
  }
  ...
  return (
    <div className="space-y-2 p-3 bg-bg-elev border border-border-subtle rounded">
      ...
      <div className="grid grid-cols-2 gap-2">
        {TIMERS.map((row) => {
          const Icon = row.IconComponent;
          if (live) {
            const killedAt =
              row.name === "Drake" ? kills.drake :
              row.name === "Herald" ? kills.herald :
              row.name === "Baron" ? kills.baron :
              null;
            const { ready, label, etaSec } = etaFor(row, killedAt, liveTime, t);
            ...
            const total = row.respawnSec || row.firstSpawnSec;
            ...
            return (
              <div
                ...
                className={`relative rounded p-2 border overflow-hidden ${
                  ready ? "border-good ring-2 ring-good/40 bg-good/5 animate-pulse" : row.ringColor
                }`}
              >
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Icon className={`w-3.5 h-3.5 ${row.color}`} />
                  <p className={`text-sm font-semibold ${row.color}`}>{row.name}</p>
                </div>
                <p className={`text-xs tabular-nums ${ready ? "text-good font-bold" : "text-white/85"}`}>
                  {ready ? t("liveTimers.spawnNow") : t("liveTimers.next", { label })}
                </p>
                {killedAt !== null && row.respawnSec > 0 && (
                  <p className="text-[9px] text-white/40 mt-0.5">
                    {t("liveTimers.lastAt", { time: formatMMSS(killedAt) })}
                  </p>
                )}
          ...
          return (
            <div
              key={row.name}
              className={`rounded p-2 border ${row.ringColor}`}
            >
              <div className="flex items-center gap-1.5 mb-0.5">
                <Icon className={`w-3.5 h-3.5 ${row.color}`} />
                <p className={`text-sm font-semibold ${row.color}`}>{row.name}</p>
              ...
              <p className="text-[11px] text-white/65 tabular-nums">
                {t("liveTimers.firstSpawn", { min: Math.round(row.firstSpawnSec / 60) })}
              </p>
              {row.respawnSec > 0 && (
                <p className="text-[11px] text-white/65 tabular-nums">
                  {t("liveTimers.respawn", { min: Math.round(row.respawnSec / 60) })}
                </p>
              )}
            </div>
          );
